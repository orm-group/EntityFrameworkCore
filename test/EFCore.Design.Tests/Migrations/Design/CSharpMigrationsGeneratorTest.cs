...
namespace Microsoft.EntityFrameworkCore.Migrations.Design
{
    public class CSharpMigrationsGeneratorTest
    {
        ...
        public void Snapshot_with_enum_discriminator_uses_converted_values()
        {
            ...
            Assert.Equal((int)RawEnum.A, snapshotModel.FindEntityType(typeof(WithAnnotations)).SqlServer().GetDiscriminatorValue());
            Assert.Equal((int)RawEnum.B, snapshotModel.FindEntityType(typeof(Derived)).SqlServer().GetDiscriminatorValue());
        }
...
", modelSnapshotCode, ignoreLineEndingDifferences: true);
        ...
        public void Snapshot_with_default_values_are_round_tripped()
        {
            ...
            foreach (var property in modelBuilder.Model.GetEntityTypes().Single().GetProperties())
            {
                ...
                Assert.Equal(property.Relational().GetDefaultValue(), snapshotProperty.Relational().GetDefaultValue());
            }
        }
